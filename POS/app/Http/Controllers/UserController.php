    ...
    public function list(Request $request)
{
    ...
    // Filter berdasarkan level_id jika tersedia
    if ($request->filled('level_id')) {
        $users->where('level_id', $request->level_id);
    }

    // Kembalikan response untuk DataTables
    return DataTables::of($users)
        ->addIndexColumn() // Menambahkan kolom nomor urut
        ->addColumn('level', function ($user) {
            return $user->level 
                ? $user->level->level_nama . ' (' . $user->level->level_kode . ')' 
                : '<span class="text-muted">Tidak ada level</span>';
        })
        ->addColumn('aksi', function ($user) {
            $id = $user->user_id;
            return '
                <button onclick="modalAction(\'' . url("/user/$id/show_ajax") . '\')" class="btn btn-info btn-sm">Detail</button>
                <button onclick="modalAction(\'' . url("/user/$id/edit_ajax") . '\')" class="btn btn-warning btn-sm">Edit</button>
                <button onclick="modalAction(\'' . url("/user/$id/delete_ajax") . '\')" class="btn btn-danger btn-sm">Hapus</button>
            ';
        })
        ->rawColumns(['level', 'aksi']) // Memberitahu bahwa 'level' & 'aksi' mengandung HTML
        ->make(true);
}
